...
const std::string shahepayGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString shahepayGUI::DEFAULT_WALLET = "~Default";

shahepayGUI::shahepayGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void shahepayGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a shahepay address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and shahepay: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to SHAHE addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your shahepay addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified shahepay addresses"));
    ...
    openAction->setStatusTip(tr("Open a shahepay: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible shahepay.sitemand-line options").arg(tr(PACKAGE_NAME)));
...
void shahepayGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void shahepayGUI::createToolBars()
{
    if(walletFrame)
    {
        /** SHAHE START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/shahepaytext")));
        ...
        QVBoxLayout* shahepayLabelLayout = new QVBoxLayout(toolbarWidget);
        shahepayLabelLayout->addWidget(label);
        shahepayLabelLayout->addWidget(toolbar);
        shahepayLabelLayout->setDirection(QBoxLayout::TopToBottom);
        shahepayLabelLayout->addStretch(1);
...
void shahepayGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool shahepayGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool shahepayGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void shahepayGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void shahepayGUI::setWalletActionsEnabled(bool enabled)
{
...
void shahepayGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void shahepayGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void shahepayGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void shahepayGUI::optionsClicked()
{
...
void shahepayGUI::showDebugWindow()
{
...
void shahepayGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void shahepayGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void shahepayGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void shahepayGUI::openClicked()
{
...
void shahepayGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void shahepayGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void shahepayGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void shahepayGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void shahepayGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void shahepayGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** SHAHE START */
void shahepayGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void shahepayGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void shahepayGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void shahepayGUI::gotoRestrictedAssetsPage()
{
...
void shahepayGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to shahepay network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void shahepayGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void shahepayGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void shahepayGUI::updateHeadersSyncProgressLabel()
{
...
void shahepayGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void shahepayGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("shahepay"); // default title
...
void shahepayGUI::changeEvent(QEvent *e)
{
...
void shahepayGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void shahepayGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void shahepayGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "SHAHE")
        msg += tr("Amount: %1\n").arg(shahepayUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(shahepayUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void shahepayGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void shahepayGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool shahepayGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool shahepayGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void shahepayGUI::setHDStatus(int hdEnabled)
{
...
void shahepayGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void shahepayGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void shahepayGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void shahepayGUI::detectShutdown()
{
...
void shahepayGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void shahepayGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void shahepayGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(shahepayGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void shahepayGUI::subscribeToCoreSignals()
{
...
void shahepayGUI::unsubscribeFromCoreSignals()
{
...
void shahepayGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void shahepayGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<shahepayUnits::Unit> units = shahepayUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const shahepayUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(shahepayUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(shahepayUnits::name(unit)));
        #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(shahepayUnits::name(u)), this);
...
{
    setText(shahepayUnits::name(newUnits));
}
...
void shahepayGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=SHAHEBTC"));
