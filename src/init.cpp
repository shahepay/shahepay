 ...
 * between shahepayd, and shahepay-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    Generateshahepays(false, 0, GetParams());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SHAHEPAY_PID_FILENAME));
#endif
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://www.shahepay.cloud>";
...
/** Sanity checks
 *  Ensure that shahepay is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single shahepay process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", SHAHEPAY_CONF_FILENAME)).string());
    ...
    Generateshahepays(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
