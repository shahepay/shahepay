...
# TestNode: A peer we use to send messages to shahepayd, and store responses.
...
class CompactBlocksTest(shahepayTestFramework):
    ...
    # This test actually causes shahepayd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that shahepayd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for shahepayd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # shahepayd will not send blocktxn responses for blocks whose height is
