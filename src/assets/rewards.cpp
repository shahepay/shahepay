...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for SHAHE
    ...
    if (p_rewardSnapshot.strDistributionAsset != "SHAHE") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is SHAHE with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using SHAHE differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "SHAHE") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse shahepay address (already validated during ownership snapshot creation)
