...
The RPM spec file provided here is for shahepay-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://shahepay.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[shahepay git repository](https://github.com/shahepayProject/shahepay) but are not part of
...
Currently only `Source10` is used, for the example `shahepay.conf` file.
...
in the shahepay git repository. At present only an SVG version of the shahepay
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the shahepay
...
needed when 0.12.1 is released, a proper fix is already in the shahepay git
master branch.
...
idea to build shahepay Core against a static version of that library compiled
...
The `shahepay.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `shahepay-server` package. At this point, most RPM based
...
The `shahepay-server` package should have SELinux support. How to properly do
...
shahepay-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/shahepay/). He has been
packaging shahepay for Fedora at least since 2012.
